...
const Message = ({ content, isUserMessage }: MessageProps) => {
  return (
    <div
      className={cn({
        "dark:bg-zinc-800": isUserMessage,
        ...
        <div className="max-w-3xl mx-auto flex items-start gap-2.5">
          ...
          <div className="flex flex-col ml-6 w-full">
            <div className="flex items-center space-x-2">
              <span className="font-sm font-semibold light:text-gray-900 dark:text-white">
                {isUserMessage ? "You" : "Website"}
              </span>
            </div>

            <p className="text-sm font-normal py-2 5 light:text-gray-900 dark:text-white">
              {content}
            </p>
          </div>
        </div>
